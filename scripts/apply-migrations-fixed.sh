...
echo "Applying migration: fix notification policies..."
...
echo "Applying migration: add projects RLS..."
curl -s -X POST "https://api.supabase.com/v1/sql" \
  -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"ref\": \"$PROJECT_REF\", \"query\": \"$(cat src/supabase/migrations/20250521000002_add_projects_rls.sql | tr '\n' ' ')\"}" > /dev/null

echo "Applying migration: create profiles table..."
curl -s -X POST "https://api.supabase.com/v1/sql" \
  -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"ref\": \"$PROJECT_REF\", \"query\": \"$(cat src/supabase/migrations/20250522000000_create_profiles_table.sql | tr '\n' ' ')\"}" > /dev/null

echo "Applying migration: fix profile relationships..."
curl -s -X POST "https://api.supabase.com/v1/sql" \
  -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"ref\": \"$PROJECT_REF\", \"query\": \"$(cat src/supabase/migrations/20250523000000_fix_profile_relationships.sql | tr '\n' ' ')\"}" > /dev/null
